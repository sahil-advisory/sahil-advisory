...
export function generateStaticParams(): Params[] {
  return referenceOf('section').map((r) => ({ slug: r.slug }))
}
export const dynamicParams = false

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { slug } = await params
  const r = getReference('section', slug)
  if (!r) return {}
        ...
        data={graph(
          {
            ...
            about: { '@type': 'DefinedTerm', name: r.name, description: r.summary, inDefinedTermSet: `${BASE_URL}/sections` },
          },
          breadcrumbJsonLd([{ name: label.plural, path: '/sections' }, { name: r.name, path }], path)
        )}
      />
